        ...
        for i in range(N):
            if i == 0:
                ...
                if time <= track[i][1]:
                    ...
                    loop_iteration = 0
                    count = 0
                    while loop_iteration < track[i][0]:
                        if count % 2 == 0:
                            loop_iteration += track[i][1]
                            count += 1
                        else:
                            loop_iteration += track[i][2]
                            count += 1
                    if (count % 2 == 1) and (loop_iteration != time):
                        if time%track[i][1] == 0:
                            time += track[i][1]
                        else:
                            time += (time%track[i][1])
                    elif (count % 2 == 0) and (loop_iteration == time):
                        time += track[i][1]
                    if N == 1:
                        time += L-track[i][0]
                        return time
